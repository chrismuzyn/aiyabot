...
class SettingsCog(commands.Cog):
    ...
    async def settings_handler(self, ctx,
        ...
        await self.settings_core(ctx,
                                 ...
                                 lora=lora,
                                 facefix=facefix,
                                 full_quality_vae=full_quality_vae,
                                 ...
                                 refresh=refresh,
                                 spoiler=spoiler,
                                 spoiler_role=spoiler_role,
                                 remove_spoiler_role=remove_spoiler_role,
                                 live_preview=live_preview)

    @commands.slash_command(name='videosettings', description='Review and change channel video defaults', guild_only=True)
    @option(
        'video_engine',
        str,
    ...
    async def video_settings_handler(self, ctx,
        ...
        await self.settings_core(ctx,
                                 ...
                                 video_init_strength=video_init_strength,
                                 max_video_frames=max_video_frames,
                                 max_video_steps=max_video_steps,
                                 max_video_size=max_video_size,
                                 video_spoiler=video_spoiler)

    async def settings_core(self, ctx,
                             current_settings: Optional[bool] = True,
                             n_prompt: Optional[str] = None,
                             data_model: Optional[str] = None,
                             steps: Optional[int] = None,
